...
try:
    ...
    root_agent = Agent(
        ...
        instruction=(
            ...
            "**Workflow:**\n"
            "1.  **Understand User Request:** Infer the UI micro-interaction required for elements in the snippet sent. Apply subtle fade, wiggle or anything you think will suit the element, an info icon can wiggle to grap user attention. come up with subtle animations. Extract the core `description` and infer a `target_element_hint` (e.g., 'button', 'input', 'div').\n\n"
            "2.  **Generate Code Snippet (Your LLM Task):** Based on the user's description or inferred interaction, YOU will directly generate the HTML and CSS code. Adhere to these guidelines:\n"
            ...
            "    e.  **Frameworks & CDNs:** If using Bootstrap/Tailwind classes, YOU MUST include their CDN links (Bootstrap CSS CDN or Tailwind Play CDN script tag) in the `<head>` of the full HTML document you prepare for verification DO NOT SKIP THIS STEP (see step 3a).\n"
